# Neon Fabs Wiki Legacy

Archived on 2026-06-24. This file preserved the prototype-era rules, tuning notes, admin notes, and design sandbox explanations before the player-facing wiki was reset.

# Neon Fabs Wiki

This was the living rules page for the local prototype. It is intentionally practical: what the player can do, what each system means, and what is still a design sandbox.
...
Admin mode exists for testing. It starts with a Test Lab: one-click scenarios that jump the local save to common playtest moments such as fresh start, Print Bay ready, first pattern ready, market ready, Merchant Dispatch, Routejack Raid, equipment install, and full inventory stress.

Below the Test Lab, Admin keeps raw debug controls grouped by purpose. It can grant items, change credits, advance time globally, tune drop rates, run battle simulations, seed markets, edit encounters, and export the current browser-local snapshot.
