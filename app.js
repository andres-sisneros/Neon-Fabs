...
function renderShipments() {
  ...
  const replayPanel = state.battleReplay?.source === "route-battle" && visibleBattles.some((battle) => battle.id === state.battleReplay.battleId)
    ? `<section class="route-replay-panel">${renderLiveBattleReplay(state.battleReplay)}</section>`
    : "";
  ...
  const merchantHistory = isMerchantView ? `
    <section class="dispatch-record-card">
  ...
  const historyCount = blocked.length + arrived.length + raided.length + intercepts.length + visibleBattles.length + state.pvpLog.length + (isRoutejackView ? state.stolenGoods.length : 0);
  const recordsOpen = Boolean(replayPanel || visibleBattles.length || blocked.length || arrived.length || raided.length || intercepts.length);
  mainPanel.innerHTML = `
    ${routeCommandPanel()}
    <section class="panel dispatch-live-panel">
      <div class="blueprint-head">
        <div>
    ...
    <details class="panel dispatch-records" ${recordsOpen ? "open" : ""}>
      <summary>
        <div>
          <h2>Records & Replays</h2>
          <p class="muted">Completed ${isRoutejackView ? "raids" : "convoys"}, encounter replays, and route logs.</p>
        </div>
        <span class="pill">${historyCount}</span>
      </summary>
      ${replayPanel}
      <div class="dispatch-record-grid">
        ${merchantHistory}
        ${routejackHistory}
        <section class="dispatch-record-card route-battle-records">
          <div class="card-row"><h2>Route Battles</h2><span class="pill">${visibleBattles.length}</span></div>
